...
class deleteUserPage extends Page{
    ...
    get clickOnAdminButton () { return $('#menu_admin_viewAdminModule');
    }
    ...
    //   const value =  await addUserPage.addRandomUserName();
       await (await this.clickOnAdminButton).waitForClickable({timeout:3000});
       await (await this.clickOnAdminButton).click();
    ...
    async clickOnDeleteButton(){
        browser.pause(3000);
        await(await this.clickDeleteButon).waitForDisplayed({timeout:3000});
        await(await this.clickDeleteButon).click();
        await(await this.okButton).waitForDisplayed({timeout:3000});
